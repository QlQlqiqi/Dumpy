...
int TimeSeriesUtil::intersectionTsSets(const vector<PqItemSeries *> *tsSet1,
                                       vector<float *> *tsSet2) {
...
int TimeSeriesUtil::intersectionTsSetsCardinality(
    const vector<PqItemSeries *> &tsSet1,
...
float TimeSeriesUtil::lb_keogh_data_bound(float *qo, float *upperLemire,
                                          float *lowerLemire, float *cb,
                                          int len, float bsf) {
  float lb = 0;
  float uu = 0, ll = 0, d = 0;
  int i = 0;
  ...
  for (i = 0; i < len1 && lb < bsf; i += 8) {
    ...
    // tu256 =
    // _mm_setr_ps(tu[order[i]],tu[order[i+1]],tu[order[i+2]],tu[order[i+3]]);
    // tl256 =
    // _mm_setr_ps(tl[order[i]],tl[order[i+1]],tl[order[i+2]],tl[order[i+3]]);
...
double TimeSeriesUtil::euclideanDist(const float *ts_1, const float *ts_2,
                                     int len) {
  double sum = 0, dp;
  for (int i = 0; i < len; i++) {
    dp = ts_1[i] - ts_2[i];
...
double TimeSeriesUtil::dtw(const float *A, const float *B, int len, int r,
                           double bsf) {
  ...
  for (k = 0; k < 2 * r + 1; k++)
    cost[k] = numeric_limits<double>::max();

  cost_prev = (double *)malloc(sizeof(double) * (2 * r + 1));
  for (k = 0; k < 2 * r + 1; k++)
    cost_prev[k] = numeric_limits<double>::max();
...
double TimeSeriesUtil::dtwsimd(const float *A, const float *B, float *cb,
                               int len, int r, float bsf, float *tSum,
