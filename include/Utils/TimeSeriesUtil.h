...
public:
  ...
  static int intersectionTsSets(const vector<PqItemSeries *> *tsSet1,
                                vector<float *> *tsSet2);
  ...
  static int
  intersectionTsSetsCardinality(const vector<PqItemSeries *> &tsSet1,
                                const vector<PqItemSeries *> &tsSet2);

  static double dtw(const float *A, const float *B, int len, int r, double bsf);

  static double dtwsimd(const float *A, const float *B, float *cb, int len,
                        int r, float bsf, float *tSum, float *pCost,
                        float *rDist);

  static bool isSame(const PqItemSeries *ts1, const float *ts2);

  static float lb_keogh_data_bound(float *qo, float *upperLemire,
                                   float *lowerLemire, float *cb, int len,
                                   float bsf);
};
